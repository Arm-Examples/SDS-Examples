...
- Clone development version of the Alif Ensemble CMSIS DFP:
  - `git clone https://github.com/VladimirUmek/alif_ensemble-cmsis-dfp.git`
- Add local pack using:
  - `cpackget add path/to/.../alif_ensemble-cmsis-dfp/AlifSemiconductor.Ensemble.pdsc`
...
> - Configure SW4 to position U4 (UART4) to see printf messages from the application.
...
- `sdsio/network/sdsio_network.clayer.yml` provides the **Ethernet Interface** for SDS file I/O
- `sdsio/usb/sdsio_usb.clayer.yml` provides the **USB Interface** for SDS file I/O (currently unused)
...
Open Terminal and type `python sdsio-server.py socket`.
...
- **Captures on-board camera stream** through SDS DataInput stream
- **Executes ML inference** using an object detection ML model
- **Captures algorithm output** through SDS DataOutput stream

Alternatively, when configured as "Player":

- **Replays pre-recorded video stream** through SDS DataInput stream
- **Executes ML inference** using an object detection ML model
- **Captures algorithm output** through SDS DataOutput stream
...
- Initializes ML model and display stream using CMSIS vStream driver
...
- Displays frames with overlaid boxes using CMSIS vStream driver
