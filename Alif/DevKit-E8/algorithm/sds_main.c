...
// SDS buffers
#ifdef SDS_PLAY
static uint8_t sds_play_buf_data_in[((ALGO_DATA_IN_BLOCK_SIZE  * 2) + 2048)] __ALIGNED(4);
#else
static uint8_t sds_rec_buf_data_in [((ALGO_DATA_IN_BLOCK_SIZE  * 2) + 2048)] __ALIGNED(4);
#endif
static uint8_t sds_rec_buf_data_out[((ALGO_DATA_OUT_BLOCK_SIZE * 2) + 2048)] __ALIGNED(4);
// SDS identifiers
#ifdef SDS_PLAY
...
__NO_RETURN void AlgorithmThread (void *argument) {
  ...
  for (;;) {
    ...
    if (sdsStreamingState == SDS_STREAMING_ACTIVE) {
      ...
      retv = sdsRecWrite(recIdDataInput, timestamp, algo_data_in_buf, sizeof(algo_data_in_buf));
      SDS_ASSERT(retv == sizeof(algo_data_in_buf));
      #endif
    }
    ...
    if (sdsStreamingState == SDS_STREAMING_ACTIVE) {
      ...
      retv = sdsRecWrite(recIdDataOutput, timestamp, algo_data_out_buf, sizeof(algo_data_out_buf));
      SDS_ASSERT(retv == sizeof(algo_data_out_buf));
    }
  }
}
