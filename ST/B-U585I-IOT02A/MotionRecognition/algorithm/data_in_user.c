...
void DiscardInputData (void) {
  ...
  /* Check for new block of accelerometer samples */
  ...
  if (((flags & osFlagsError)          == 0U) &&        // If not an error and
    ...
    /* Release accelerometer samples block */
    if (ptrDriver_vStreamAccel->ReleaseBlock() != VSTREAM_OK) {
      SDS_PRINTF("Failed to release accelerometer samples block\n");
    }
  }
}
